...
describe('FixedPoint', () => {
    ...
    test('should create a fixed-point number from a whole number', () => {
        const val = 10;
        const fpVal = fp(val);
        expect(fpVal.getRaw()).toBe(val * SCALE);
    ...
    test('should create a fixed-point number from a fractional number', () => {
        const val = 10.5;
        const fpVal = fp(val);
        expect(fpVal.getRaw()).toBe(Math.trunc(val * SCALE));
    ...
    test('should throw an error when taking the square root of a negative number', () => {
        const fp1 = fp(-16.0);
        expect(() => fp1.sqrt()).toThrow('FixedPoint square root of negative number.');
    });
    ...
    test('should compare two fixed-point numbers correctly', () => {
        ...
        expect(fp1.greatherThanOrEqualTo(fp3)).toBe(true);
