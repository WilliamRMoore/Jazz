(() => {
  ...
  var STATES = class {
    ...
    SPOT_DOGE_S = seq.Next;
    ROLL_DODGE_S = seq.Next;
  };
  ...
  function InitShieldRaiseRelations() {
    ...
    shieldRaiseTranslations.SetConditions([ToSpotDodge]);
  ...
  function InitShieldRelations() {
    ...
    translations.SetConditions([shieldToShieldDrop, ToSpotDodge]);
  ...
  function InitSpotDodgeRelations() {
    ...
    translations.SetDefaults([defaultIdle]);
    return new StateRelation(STATE_IDS.SPOT_DOGE_S, translations);
  }
  ...
  var DefaultCharacterConfig = class {
    ...
    constructor() {
      ...
      this.attacks.set(ATTACK_IDS.N_GRND_ATK, neutralAttack).set(ATTACK_IDS.D_TILT_ATK, downTilt).set(ATTACK_IDS.U_TILT_ATK, upTilt).set(ATTACK_IDS.S_TILT_ATK, sideTilt).set(ATTACK_IDS.S_TILT_U_ATK, sideTiltUp).set(ATTACK_IDS.S_TITL_D_ATK, sideTiltDown).set(ATTACK_IDS.S_CHARGE_ATK, sideCharge).set(ATTACK_IDS.S_CHARGE_EX_ATK, sideChargeExtension).set(ATTACK_IDS.U_CHARGE_ATK, upCharge).set(ATTACK_IDS.U_CHARGE_EX_ATK, upChargeExtension).set(ATTACK_IDS.D_CHARGE_ATK, downCharge).set(ATTACK_IDS.D_CHARGE_EX_ATK, downChargeEx).set(ATTACK_IDS.N_SPCL_ATK, nSpecial).set(ATTACK_IDS.S_SPCL_ATK, sideSpecial).set(ATTACK_IDS.S_SPCL_EX_ATK, sideSpecialEx).set(ATTACK_IDS.D_SPCL_ATK, downSpecial).set(ATTACK_IDS.D_SPCL_AIR_ATK, downSpecialAerial).set(ATTACK_IDS.N_AIR_ATK, neutralAir).set(ATTACK_IDS.F_AIR_ATK, fAir).set(ATTACK_IDS.U_AIR_ATK, uAir).set(ATTACK_IDS.B_AIR_ATK, bAir).set(ATTACK_IDS.D_AIR_ATK, dAir).set(ATTACK_IDS.S_SPCL_AIR_ATK, sideSpecialAir).set(ATTACK_IDS.S_SPCL_EX_AIR_ATK, sideSpecialExAir).set(ATTACK_IDS.DASH_ATK, dashAtk).set(ATTACK_IDS.U_SPCL_ATK, upSpecial);
    }
    populateHurtCircles() {
  ...
  function GetFAir() {
    ...
    const FairAttack = new AttackBuilder("FAir").WithTotalFrames(fairTotalFrames).WithInteruptableFrame(fairTotalFrames).WithBaseKnockBack(fairBaseKnockback).WithKnockBackScaling(45).WithGravity(true).WithHitBubble(fairDamage, fairRadius, 2, fairLaunchAngle, bubble1Offsets).WithHitBubble(fairDamage, fairRadius, 1, fairLaunchAngle, bubble2Offsets).WithHitBubble(fairDamage, fairRadius, 0, fairLaunchAngle, bubble3Offsets).Build();
    return FairAttack;
  }
  function GetBAir() {
  ...
  function GetNSpecial() {
    ...
    bldr.WithTotalFrames(activeFrames).WithGravity(true).WithHitBubble(h1Damage, radius, 0, 65, h1Offset).WithBaseKnockBack(baseKb).WithKnockBackScaling(knockBackScaling);
    return bldr.Build();
  }
  function GetSideSpecial() {
    ...
    bldr.WithUpdateAction(onUpdate).WithExitAction(onExit).WithEnterAction(onEnter).WithImpulses(impulses, 13).WithTotalFrames(activeFrames).CanOnlyFallOffLedgeIfFacingIt().WithGravity(false);
    return bldr.Build();
  }
  ...
  function GetUpSpecial() {
    ...
    bldr.WithTotalFrames(totalFrameLength).WithImpulses(impulses, 12).WithGravity(false).WithEnterAction((w, p) => {
      p.Velocity.X = 0;
      p.Velocity.Y = 0;
    });
    return bldr.Build();
  ...
  function PlayerShields(pd, localFrame) {
    ...
    for (let i = 0; i < playerCount; i++) {
      ...
      if (shield.Active) {
        ...
        const triggerValue = input.LTVal >= input.RTVal ? input.LTVal : input.RTVal;
  ...
  function PAvsPB(currentFrame, activeHbPool, atkResPool, vecPool, colResPool, clstsPntsResPool, componentHistories, pA, pB) {
    ...
    if (pB.Shield.Active) {
      ...
      const shieldPos = vecPool.Rent().SetXY(pB.Position.X, pB.Position.Y + shieldYOffset);
  ...
  function CalculateLaunchVector(vecPool, launchAngle, isFacingRight, knockBack) {
    let angleInRadians = launchAngle * (Math.PI / 180);
    if (isFacingRight === false) {
      angleInRadians = Math.PI - angleInRadians;
    }
    return vecPool.Rent().SetXY(
